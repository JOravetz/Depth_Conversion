...
int main (int argc, char **argv) {
   ...
   for ( i = 0; i < ntr; ++i ) {
      ...
      itop = min ( max ( nint ( twt_dv_array[0] / dt ), 0 ), ns );
      gradient = dv_array[0] / (double) itop;
      if ( verbose == 3 ) fprintf ( stderr, "twt_dv_array = %f, dt = %f, itop = %5d, gradient = %f, dv_array = %f\n", twt_dv_array[0], dt, itop, gradient, dv_array[0] );
      for ( j = 0; j < itop; ++j ) {
         if ( verbose == 3 ) fprintf ( stderr, "j = %5d, gradient * j = %f, gradient = %f\n", j, gradient * j, gradient );
         tr.data[j] += gradient * (double) j;
      }

      ibottom = min ( max ( nint ( twt_dv_array[ntm1] / dt ), 0 ), ns );
      gradient = dv_array[ntm1] / (double) ( ns - ibottom );
      if ( verbose == 3 ) fprintf ( stderr, "twt_dv_array = %f, dt = %f, ibottom = %5d, gradient = %f, dv_array = %f\n", twt_dv_array[ntm1], dt, ibottom, gradient, dv_array[ntm1] );
      for ( j = ibottom+1; j < ns; ++j ) 
         if ( verbose == 3 ) fprintf ( stderr, "j = %5d, gradient * (ns - j - 1) = %f, gradient = %f\n", j, gradient * (ns-j-1), gradient );
         tr.data[j] += gradient * (double) (ns - j - 1);

      for ( j = itop; j <= ibottom; ++j ) {
         time = twt_array[j];
         dintlin ( nt, twt_dv_array, dv_array, dv_array[0], dv_array[ntm1], 1, &time, &dv_interp);
         if ( verbose == 3 && ( j == itop || j == ibottom ) ) fprintf ( stderr, "j = %d, itop = %d, time = %f, dv_interp = %f\n", j, itop, time, dv_interp );
         tr.data[j] += dv_interp;
      }
      puttr ( &tr ); 
   }
