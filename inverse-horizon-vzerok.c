...
int main (int argc, char **argv) {
   ...
   short  verbose;
   double water_depth, vzero, factor, x_loc, y_loc;
   double value_coeff_x, value_coeff_x2, twt;
   double z_horizon;
   ...
   while (((char *) NULL) != fgets ( temp, sizeof(temp), stdin )) {
      (void) sscanf ( ((&(temp[0]))), "%lf%lf%lf", &x_loc, &y_loc, &z_horizon );
      ...
      if ( x_loc >= grd_x.x_min && x_loc <= grd_x.x_max && y_loc >= grd_x.y_min && y_loc <= grd_x.y_max ) {
	    ...
	    if ( verbose == 2 ) fprintf ( stderr, "X-Loc = %f, Y-Loc = %f, X Coefficient = %0.10f, X2 Coefficient = %0.10f\n", x_loc, y_loc, value_coeff_x, value_coeff_x2 );
            ...
            if ( z_horizon <= water_depth ) {
               twt = z_horizon / ( factor * vzero );
            } else {
               twt = log ( ( -value_coeff_x2 * water_depth + value_coeff_x2 * z_horizon + vzero ) / vzero ) / ( value_coeff_x2 * factor ) + value_coeff_x;
            }

            if ( verbose == 2) fprintf ( stderr, "xloc = %8.2f, yloc = %8.2f, WB TWT (msec) = %8.4f, k = %10.6f, horizon Z (meters) = %10.4f, TWT value = %10.4f\n", x_loc, y_loc, value_coeff_x, value_coeff_x2, z_horizon, twt ); 
