   ...
   if ( tr.gelev >= gelev_globalmin && tr.gelev <= gelev_globalmax && tr.selev >= selev_globalmin && tr.selev <= selev_globalmax ) {
      ...
      for ( k = 0; k < ns; ++k ) {
         if ( depth[k] > depth[k-1] ) {
            ++num;
            tr_amp[num] = tr.data[k];
            depth_new[num] = depth[k];
         }
      }
      ...
      for ( k=0; k < nz1; ++k ) {
         z = ( k * dt ) + dfirst;
         if ( z <= depth_max ) {
            dintlin ( num+1, depth_new, tr_amp, tr_amp[0], tr_amp[num], 1, &z, &amp_out );
