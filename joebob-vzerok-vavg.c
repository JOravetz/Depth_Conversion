...
int main (int argc, char **argv) {
   ...
   short  verbose;
   int    scalar, ntr, ns;
   double water_depth, vzero, ratio, factor1, x_loc, y_loc;
   double value_coeff_x, value_coeff_x2, tr_msec, dt_msec;
   float depth;
   register int k, n;
   ...
   for ( k = 0; k < ntr; ++k ) {
      ...
      if ( x_loc >= grd_x.x_min && x_loc <= grd_x.x_max && y_loc >= grd_x.y_min && y_loc <= grd_x.y_max ) {
	    ...
	    if ( verbose == 2 ) fprintf ( stderr, "Trace num = %d, X-Loc = %f, Y-Loc = %f, X Coefficient = %0.10f, X2 Coefficient = %0.10f\n", k+1, x_loc, y_loc, value_coeff_x, value_coeff_x2 );
            ...
            water_depth = value_coeff_x * factor1 * vzero;
            ratio = vzero / value_coeff_x2;
	    for ( n=0; n < ns; ++n ) {
	       tr_msec   = n * dt_msec;
	          ...
	          tr_msec = ( tr_msec - value_coeff_x ) * factor1;
                  depth = ( ratio * (exp (value_coeff_x2*tr_msec) - 1.0) ) + water_depth;
               ...
               if ( verbose == 2)  {
                  ...
                  k, x_loc, y_loc, value_coeff_x, value_coeff_x2, tr_msec, n, depth, tr.data[n] ); 
	       }
	    }
            tr.trid = 1;
         }
