...
int main (int argc, char **argv) {
   ...
   if ( verbose ) fprintf ( stderr, "Initialize num_dv and data_dv storage arrays\n" );
   for ( i = 0; i < nx; ++i ) {
      for ( j = 0; j < ny; ++j ) {
         ilive[i][j] = 0;
         for ( k = 0; k < ns; ++k ) {
   ...
   kount = -1;
   rewind ( fpp );
   if ( verbose ) fprintf ( stderr, "Start reading in correction values to determine min and max values\n" );
   ...
   rewind ( fpp );
   if ( verbose ) fprintf ( stderr, "Read in the actual data here\n" );
   for ( i = 0; i <= kount; ++i ) {
      ...
      if ( verbose ) fprintf ( stderr, "Finished reading line = %5d\n", i );
   }
   efclose ( fpp );

   if ( verbose ) fprintf ( stderr, "Finished reading in residual delta-v corrections file\n" );

   for ( k = 0; k <= kount; ++k ) {
      if ( verbose ) fprintf ( stderr, "Working on line correction = %5d\n", k );
      iloc = jloc = 0;
