...
int main(int argc, char **argv) {
   ...
   while (NULL != fgets ( temp, sizeof(temp), fpp )) {
      ++kount;
      (void) sscanf ( ((&(temp[0]))), "%s%lf%lf%lf%lf", well, &x_loc, &y_loc, &num, &sample );
   }

   if ( verbose != 0) {
      fprintf (stderr,"\n");
      fprintf (stderr,"Working on Well = %s\n", well );
   ...
   for ( i=0; i<=kount; i++ ) {
      ...
      if ( verbose ) {
         ...
         fprintf ( stderr, "%-25s %12.2f %12.2f ", well, xloc_array[i], yloc_array[i] );
         fprintf ( stderr, "%30.25f %30.25f %8.2f %8.2f %8.2f\n", vzero, k, wb_twt_array[i], wb_z_array[i], sum / (float) nump1 );
      }
      printf ( "%-25s %12.2f %12.2f ", well, xloc_array[i], yloc_array[i] );
