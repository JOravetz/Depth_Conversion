...
int main (int argc, char **argv) {
   ...
   x_loc_array   = ealloc1double ( kount );
   y_loc_array   = ealloc1double ( kount );
   twt_array     = ealloc1double ( kount );
   delta_v_array = ealloc1double ( kount );
   ...
   #pragma omp parallel private (i,j,k,dist_min,dist,iloc,jloc,kloc)
{
   ...
   int num;
   double **dv_interp, sum;

   dv_interp = ealloc2double ( ny, nx );

   /* all the data read in - now let's loop thru the populated delta_v corrections */
   for ( k = 0; k < ns; ++k ) {
      num = -1;
      sum = 0.0;
      for ( i = 0; i < nx; ++i ) {
